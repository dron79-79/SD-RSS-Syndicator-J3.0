...
class media_group extends HtmlDescribable {
	var $content_url;
	var $player_url;
	var $thumbnail_url;
	var $additionalElements = Array();
...
class FeedDate {
	...
	function iso8601() {
		...
		if ($this->tzoffset2!="") $date = str_replace("+00:00",$this->tzoffset2,$date);
		return $date;
	}
...
/**
	...
	function createFeed() {
		...
		//$feed.= "<rss version=\"".$this->RSSVersion."\"  xmlns:atom=\"http://www.w3.org/2005/Atom\">\n";
		$feed.= '<rss xmlns:yandex="http://news.yandex.ru" xmlns:media="http://search.yahoo.com/mrss/" version="2.0">
		';
		$feed.= "    <channel>\n";
		...
		$feed.= "        <description>".$this->getDescription2()."</description>\n";
		//$now = new FeedDate( );
		//$feed.= "        <lastBuildDate>".htmlspecialchars($now->rfc822())."</lastBuildDate>\n";
		//$feed.= "        <generator>". $this->version()."</generator>\n";
		...
		if ($this->image!=null) {
			$feed.= "        <image>\n";
			$feed.= "            <url>".htmlspecialchars(htmlspecialchars_decode($this->image->url))."</url>\n";
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			*/
			...
			$feed.= $this->_createAdditionalElements($this->items[$i]->additionalElements, "        ");
			if (sizeof($this->items[$i]->media_group)) {
				foreach ($this->items[$i]->media_group as $mg)
				{
					$feed.= "            <media:group>";
					$feed.= '<media:content url="'.htmlspecialchars($mg->content_url,ENT_QUOTES,'UTF-8',false).'"/>';
					if(!empty($mg->player_url)){
						$feed.= '<media:player url="'.htmlspecialchars($mg->player_url,ENT_QUOTES,'UTF-8',false).'"/>';
					}
					if(!empty($mg->thumbnail_url)){
						$feed.= '<media:thumbnail url="'.htmlspecialchars($mg->thumbnail_url,ENT_QUOTES,'UTF-8',false).'"/>';
					}
					$feed.= "</media:group>\n";
				}
